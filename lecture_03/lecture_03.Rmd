...
But we want to solve the inverse problem: we already know the stationary distribution we want (the Boltzmann distribution) and we need to find transition probabilities that will produce it.

For the 2-state weather model, the balance equation involves only one pair of states, so it can be rearranged into a clean ratio: $P(\text{sunny} \to \text{rainy})/P(\text{rainy} \to \text{sunny}) = \pi_\text{rainy}/\pi_\text{sunny}$. Any transition probabilities satisfying this ratio will work. But for three or more states, each global balance equation involves *all* states—the equation for sunny includes transitions to and from rainy *and* cloudy. The equations are coupled: each equation involves the equilibrium probability of *every* state. For a chemical system, where each $\pi_i \propto \exp(-U_i/kT)$, this means we need to know the energy of every state—exactly the exhaustive calculation we are trying to avoid. We need a more tractable approach.
...
Each pair of states gives one independent constraint on the *ratio* of transition probabilities—we only ever need to consider two states at a time. For the Boltzmann distribution, $\pi_i \propto \exp(-U_i/kT)$, so:
...
Detailed balance provides a tractable alternative. By requiring that probability flow balances for each *pair* of states independently, we only ever need the energy difference between two states at a time: $P(i \to j)/P(j \to i) = \exp(-(U_j - U_i)/kT)$. The Metropolis acceptance criterion satisfies this constraint: always accept moves that lower the energy; accept energy-raising moves with probability $\exp(-\Delta U/kT)$. No partition function is needed.
