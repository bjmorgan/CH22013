...
In the [previous lecture](monte-carlo-chemical), we considered the problem of effective sampling of chemical systems. Uniform sampling—where we select configurations from the entire configuration space with equal probabilities—is typically highly inefficient and often computationally intractable. Uniform sampling predominantly generates physically irrelevant high-energy configurations, while the low-energy states that contribute the most to thermodynamic averages occupy only a tiny fraction of configuration space. For even modestly sized molecules, the vast majority of possible configurations have such high energies that their Boltzmann weights are effectively zero.

As a first step to addressing this problem, we introduced the concept of generating a sequence of samples, where each new configuration comes from applying a small, local change to the previous one. The key insight was that once we reach a high-probability region of configuration space, local moves tend to sample other high-probability configurations rather than producing highly unphysical structures.
The mathematical foundation for this approach is the Markov chain, which provides a framework for generating this stochastic path through the set of all possible configurations. A crucial property of Markov chains is that, when properly constructed, they eventually reach a stationary distribution—the frequency with which they visit different states stabilises to a consistent pattern.

In the context of computational chemistry, we want to be able to design Markov chains whose stationary distribution matches the Boltzmann distribution. When we achieve this, our simulation will naturally spend time in different molecular configurations proportional to their physical probabilities, allowing us to compute thermodynamic averages through simple averaging.^[Direct sampling from the Boltzmann distribution also provides maximum statistical efficiency—we get the lowest possible variance in our estimates for a fixed number of samples.]
