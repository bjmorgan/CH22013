...
In Lecture \@ref(monte-carlo-chemical), we built a weather model as a Markov chain: given a set of transition probabilities (the chances of going from sunny to rainy, rainy to cloudy, and so on), we discovered what stationary distribution those probabilities produce. But we want to solve the inverse problem: we already know the stationary distribution we want (the Boltzmann distribution) and we need to find transition probabilities that will produce it.

### Global balance

At equilibrium, the probability of being in each state does not change over time. If this were not the case, the distribution would still be evolving and the system would not be at equilibrium. For the distribution to remain constant, the total probability flow into each state must equal the total flow out. This condition is called "global balance."
...
To make this concrete, consider a weather model with three states: sunny, rainy, and cloudy. For each state, global balance requires that the total probability flow out equals the total probability flow in:
...
These three global balance equations (one per state), together with the normalisation requirement that $\pi_\text{sunny} + \pi_\text{rainy} + \pi_\text{cloudy} = 1$, determine the stationary distribution. However, only two of the three global balance equations are independent (if probability flow balances for two states, it must balance for the third). So we have two independent equations plus the normalisation constraint, which is exactly enough to solve for the three unknown probabilities $\pi_i$.

This tells us that global balance relates the *complete set* of transition probabilities to the stationary distribution. For the inverse problem, we would need to work backwards: starting from a desired distribution, find a full set of transition probabilities that produce it. For a system with $N$ states, this means determining $N^2$ transition probabilities (the full transition matrix), which is a difficult problem to solve directly. We need a more tractable approach.

### Detailed balance

Instead of requiring that probability flows balance *globally* (the total in equals the total out for each state), we can impose a stronger condition: that probability flows balance *pairwise*. This condition, called "detailed balance," requires that for each pair of states $i$ and $j$, the probability flow from $i$ to $j$ exactly equals the flow from $j$ back to $i$:

$$
\pi_i \times P(i \to j) = \pi_j \times P(j \to i)
$$

where $\pi_i$ is the equilibrium probability of state $i$ and $P(i \to j)$ is the transition probability from state $i$ to state $j$.

If detailed balance holds for every pair, then global balance is automatically satisfied: the total flow out of any state $i$ equals $\sum_j \pi_i P(i \to j)$, and summing the pairwise balance conditions over all $j$ shows this equals the total flow in $\sum_j \pi_j P(j \to i)$.^[Detailed balance is sufficient for global balance, but not necessary. There are Markov chains that satisfy global balance without satisfying detailed balance.]

To see how this works concretely, consider the 2-state weather model from Lecture \@ref(monte-carlo-chemical):

```{r detailed-balance-weather, fig.cap="Detailed balance applied to the 2-state weather model from Lecture \\@ref(monte-carlo-chemical). The pairwise balance condition $\\pi(\\text{sunny}) \\times 0.3 = \\pi(\\text{rainy}) \\times 0.4$, together with the normalisation constraint, uniquely determines the stationary distribution.", out.width='80%', fig.align='center'}
knitr::include_graphics("lecture_03/figures/global_balance1.png")
```

There is only one pair of states, so detailed balance gives us one equation: $\pi_\text{sunny} \times 0.3 = \pi_\text{rainy} \times 0.4$. Combined with $\pi_\text{sunny} + \pi_\text{rainy} = 1$, this gives $\pi_\text{sunny} = 4/7$ and $\pi_\text{rainy} = 3/7$, exactly the stationary distribution we found in Lecture \@ref(monte-carlo-chemical). For larger systems, each pair of states provides one detailed balance equation constraining the *relative* probabilities of those two states. Applied consistently to all pairs, these pairwise constraints determine the entire stationary distribution.

### Why detailed balance helps with the inverse problem

The advantage of detailed balance for the inverse problem is that it decomposes a global constraint (the full transition matrix must produce the desired stationary distribution) into independent pairwise constraints. We can rearrange the detailed balance equation:
...
If we know the ratio $\pi_j/\pi_i$ for our target distribution, we know the ratio $P(i \to j)/P(j \to i)$ that the transition probabilities must satisfy. For the Boltzmann distribution, $\pi_i \propto \exp(-U_i/kT)$, so:

$$
\frac{P(i \to j)}{P(j \to i)} = \frac{\exp(-U_j/kT)}{\exp(-U_i/kT)} = \exp(-(U_j-U_i)/kT)
$$

The ratio of transition probabilities depends only on the *energy difference* between the two states, not on the absolute energies or the partition function. This is what makes the approach practical: we do not need to know the normalisation constant $Z$ to construct our Markov chain.
...
- If the proposed move *raises* the energy ($U_j > U_i$), accept with probability $\text{acc}(i \to j) = \exp(-(U_j - U_i)/kT)$.

We can check this satisfies detailed balance. For the case $U_j > U_i$ (an uphill move from $i$ to $j$): $\text{acc}(i \to j) = \exp(-(U_j - U_i)/kT)$ and $\text{acc}(j \to i) = 1$ (since the reverse move is downhill). The ratio is $\exp(-(U_j - U_i)/kT)$, which matches the detailed balance requirement. The case $U_i > U_j$ gives the same result by symmetry.
...
6. **Acceptance decision**: Apply the Metropolis criterion to decide whether to accept the proposed move:
   ...
   This use of a random number implements probabilistic acceptance: comparing $\xi$ to $\exp(-\Delta U/kT)$ means we accept the move with exactly that probability.
...
For concreteness, set $E_2 - E_1 = kT$, so $P(2)/P(1) = e^{-1} \approx 0.37$—state $1$ should appear about 73% of the time. Starting in state $1$, the only possible move is to state $2$, which requires an energy increase of $kT$. The Metropolis acceptance probability is $\exp(-1) \approx 0.37$, so roughly two-thirds of the time this move is rejected. When we correctly record state $1$ again after each rejection, state $1$ accumulates the extra counts it needs to reflect its higher Boltzmann weight.
...
$$
\langle A \rangle = \frac{\sum_i A_i \exp(-E_i/kT)}{\sum_i \exp(-E_i/kT)}
$$

This requires enumerating every possible state and computing the partition function $Z = \sum_i \exp(-E_i/kT)$.
...
In Lecture \@ref(monte-carlo-chemical), we saw that a Markov chain with the right properties converges to a unique stationary distribution. In this lecture, we tackled the inverse problem: given that we want the Boltzmann distribution as our stationary distribution, what transition probabilities should we use?

Detailed balance (the condition that probability flow between every pair of states is equal at equilibrium) provides a sufficient condition for a Markov chain to have a given stationary distribution. Setting the target distribution to Boltzmann, detailed balance constrains the ratio of forward and reverse transition probabilities to $P(i \to j)/P(j \to i) = \exp(-(U_j - U_i)/kT)$. The Metropolis acceptance criterion satisfies this constraint: always accept moves that lower the energy; accept energy-raising moves with probability $\exp(-\Delta U/kT)$. Only energy differences are needed, not the partition function.
