...
Some variables can only take specific, countable values. Rolling a fair six-sided die, for example, gives each number (1&ndash;6) a probability of 1/6 (Figure \@ref(fig:single-die)). This is a *uniform discrete distribution*: every outcome is equally likely.
...
Not all discrete distributions are uniform: counting heads in four coin flips gives probabilities of 1/16, 4/16, 6/16, 4/16, and 1/16 for 0, 1, 2, 3, and 4 heads respectively, because there are more ways to get 2 heads than 0 or 4 (Figure \@ref(fig:two-coins)).
...
Other variables can take any value within a continuous range. The Maxwell–Boltzmann speed distribution (Figure \@ref(fig:maxwell-boltzmann)), for example, describes the probability of finding a molecule travelling at a particular speed. For these *continuous distributions*, the probability of finding the variable in a specific range equals the area under the curve over that range.
...
\begin{equation}
Z = \sum_{\mathbf{r}} \exp(-U(\mathbf{r})/kT)
\end{equation}
...
The obstacle lies in the sheer number of possible configurations for any realistic chemical system. Consider a simple example: a $10 \times 10$ array of surface sites where we want to model the adsorption of 50 molecules (Figure \@ref(fig:single-lattice)).
...
This system has approximately $10^{29}$ possible configurations—more than the number of stars in the observable universe (Figure \@ref(fig:many-lattices)). Direct enumeration becomes impossible.
...
Our second example involves calculating the average value of a function over a specific interval. Consider finding the average value of $\sin^2(x)$ over the interval $[0,\pi]$ (Figure \@ref(fig:sin2x)).
