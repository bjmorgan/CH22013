...
$$
\langle A \rangle = \sum_i A_i \times P_i = \sum_i A_i \times \frac{\exp(-U_i/kT)}{Z} = \frac{1}{Z}\sum_i A_i \exp(-U_i/kT)
$$

If we sample $N$ configurations uniformly, the numerator is estimated by $\frac{1}{N}\sum_{i=1}^N A(r_i) \exp(-U(r_i)/kT)$ and the partition function $Z$ itself is estimated by $\frac{1}{N}\sum_{i=1}^N \exp(-U(r_i)/kT)$. Taking the ratio, the factors of $1/N$ cancel, giving:
